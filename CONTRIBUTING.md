...
1. **Submit RFC**: Create a [new GitHub issue](https://github.com/cosai-oasis/ws4-secure-design-agentic-systems/issues/new?template=rfc-template.md) with the `rfc` label. This will automatically mark it as `In Review`. You can find the [template here](rfc-template.md). The RFC should describe the current gap or topic, and may be broad, narrow, or abstract.

2. **_(Optional)_ Submit a Pull Request**: The issue identifies the gap, while the pull request will formalize definition. When the RFC is proposing a solution, specification, or scope, the PR will merge in `RFC/RFC-{num}.md` into `main`. This gives us one authoritative place where the RFC's scope and status live, reviewable by direct suggestion and diff rather than by comment thread in the GitHub issue which can be lost.

3. **Approval**: Approved RFCs will be:
   ...
   - Work should assign a named lead or leads for the work. 
   - Work can proceed in a separate branch and merged into `main` following the necessary TSC and WS lead reviews. 
   - In some instances acceptance of the RFC PR may conclude the work.

To gain support and collaboration from the community, we recommend taking the following actions during the process.

**Notify Community**: Post notifications on:
...
**Present RFC**: Time will be allocated to present your RFC at the next available meeting.

**Community Review**: After presentation, at least one additional week will be provided for community feedback. Gain the support for other CoSAI members. 
